...
void ViewFrustum::setCamDef(glm::vec3 &p_v, glm::vec3 &l_v, glm::vec3 &u_v) {

    Vec3 p = Vec3(p_v.x, p_v.y, p_v.z);
    Vec3 l = Vec3(l_v.x, l_v.y, l_v.z);
    Vec3 u = Vec3(u_v.x, u_v.y, u_v.z);
...
int ViewFrustum::pointInFrustum(glm::vec3 &p_v) {
    Vec3 p = Vec3(p_v.x, p_v.y, p_v.z);
	int result = INSIDE;
	for(int i=0; i < 6; i++) {
...
int ViewFrustum::sphereInFrustum(const glm::vec3 &p_v, float raio) {
    Vec3 p = Vec3(p_v.x, p_v.y, p_v.z);
	int result = INSIDE;
	float distance;
