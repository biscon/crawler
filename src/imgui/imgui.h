...
struct ImDrawList;                  // A single draw command list (generally one per window, conceptually you may see this as a dynamic "geometryMesh" builder)
...
struct ImGuiStyle
{
    ...
    ImGuiDir    WindowMenuButtonPosition;   // CellSide of the collapsing/docking button in the title bar (None/Left/Right). Defaults to ImGuiDir_Left.
    ...
    ImGuiDir    ColorButtonPosition;        // CellSide of the color button in the ColorEdit4 widget (left/right). Defaults to ImGuiDir_Right.
...
// It is not done by default because they are many perfectly useful way of altering render state for imgui contents (e.g. changing geometryShader/blending settings before an Image call).
...
struct ImDrawList
{
    ...
    ImDrawListFlags         Flags;              // Flags, you may poke into these to adjust anti-aliasing settings per-operation.
