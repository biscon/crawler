...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            // FIXME-TABLE: Increase minimum size during create frame to avoid biasing auto-fitting widgets
    ...
    // This path will only be exercised when sort specs are modified before header rows (e.g. create or visibility change)
